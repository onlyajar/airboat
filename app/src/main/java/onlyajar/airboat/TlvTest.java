...
import onlyajar.airboat.tlv.HexUtils;
import onlyajar.airboat.tlv.TlvBuilder;
import onlyajar.airboat.tlv.TlvData;
import onlyajar.airboat.tlv.TlvParser;
...
public class TlvTest {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("编码结果: " + HexUtils.bytesToHex(data));

        System.out.println("\n========== 2. 解析测试 ==========\n");

        List<TlvData> tlvs = TlvParser.parse(data);
        ...
        // 按标签查找
        TlvData found = TlvParser.findByTag(TlvParser.parse(data), "9F26");
        if (found != null) {
            System.out.println("找到 9F26 = " + found.getValueHex());
        }

        // 嵌套查找
        TlvData nested = TlvParser.findByTag(TlvParser.parse(data), "9F6C");
        if (nested != null) {
            System.out.println("找到 9F6C (嵌套) = " + nested.getValueHex());
        }

        System.out.println("\n========== 4. 长度编码边界测试 ==========\n");
        TlvProtocol berTlvProtocol = new BerTlvProtocol();
        // 测试不同长度的编码
        int[] testLengths = {0, 1, 127, 128, 255, 256, 65535, 65536};
        for (int len : testLengths) {
            byte[] encoded = berTlvProtocol.encodeLength(len);
            System.out.printf("长度 %6d → %s%n", len, HexUtils.bytesToHex(encoded));
        }

        System.out.println("\n========== 5. 往返一致性测试 ==========\n");

        // 编码 → 解析 → 重新编码，结果应完全一致
        byte[] roundTrip = new TlvBuilder()
        ...
        List<TlvData> parsed = TlvParser.parse(roundTrip);
        TlvBuilder reBuilder = new TlvBuilder();
        for (TlvData tlv : parsed) {
            reBuilder.add(tlv);
        }
        byte[] rebuilt = reBuilder.build();

        System.out.println("原始编码: " + HexUtils.bytesToHex(roundTrip));
        System.out.println("重建编码: " + HexUtils.bytesToHex(rebuilt));
